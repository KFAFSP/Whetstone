...
namespace Whetstone.Contracts
{
    ...
    public static class Optional
    {
        ...
        /// <returns>An <see cref="Optional{T}"/> wrapping the value <paramref name="AValue"/>.</returns>
    ...
    /// <remarks>
    /// The <see cref="Optional{T}"/> is an <see cref="SCG.IEnumerable{T}"/> that is either empty or contains exactly the wrapped value.
    /// </remarks>
    [PublicAPI]
        ...
        SCG.IEnumerable<T>
    {
        ...
        /// <summary>
        /// Check whether that present value satisfies a specified <see cref="Predicate{T}"/> and propagate it; otherwise propagate absence.
        /// </summary>
        /// <param name="APredicate">The <see cref="Predicate{T}"/> to check for, may not be <see langword="null"/>.</param>
        /// <returns>An <see cref="Optional{T}"/> that wraps the matching value; otherwise absent.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="APredicate"/> is <see langword="null"/>.</exception>
        ...
        /// </summary>
        /// <remarks>
        /// Execution stops when <paramref name="AAction"/> throws any <see cref="Exception"/>, which is then propagated to the caller.
        /// </remarks>
        /// <param name="AAction">The <see cref="Action{T}"/> to execute on the value, may not be <see langword="null"/>.</param>
        /// <exception cref="ArgumentNullException"><paramref name="AAction"/> is <see langword="null"/>.</exception>
        /// <exception cref="Exception">Any <see cref="Exception"/> thrown by <paramref name="AAction"/>.</exception>
        ...
        /// </summary>
        /// <remarks>
        /// Execution stops when <paramref name="AFunc"/> throws any <see cref="Exception"/>, which is then propagated to the caller.
        /// </remarks>
        /// <typeparam name="TOut">The type of the output.</typeparam>
        /// <param name="AFunc">The <see cref="Func{T, TOut}"/> to compute on the value, may not be <see langword="null"/>.</param>
        /// <returns>An <see cref="Optional{TOut}"/> with the result present, or absent if the value is absent.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="AFunc"/> is <see langword="null"/>.</exception>
        /// <exception cref="Exception">Any <see cref="Exception"/> thrown by <paramref name="AFunc"/>.</exception>
        ...
        [Pure]
        public T OrDefault([CanBeNull] [NoEnumeration] T ADefault = default(T)) => IsPresent ? FValue[0] : ADefault;

        /// <summary>
        /// Performs an <see langword="as"/> cast of the value if present; otherwise propagates absence.
        /// </summary>
        /// <typeparam name="TOut">The type to cast to.</typeparam>
        /// <returns>An <see cref="Optional{TOut}"/> with the cast result present, or absent if the value is absent.</returns>
        [Pure]
        ...
        /// </summary>
        /// <typeparam name="TOut">The type to cast to.</typeparam>
        /// <returns>An <see cref="Optional{TOut}"/> with the non-<see langword="null"/> cast result of the present value; otherwise absent.</returns>
        [Pure]
            ...
            where TOut : class
        {
            ...
            var tValue = FValue[0] as TOut;
            return tValue != null
                ? new Optional<TOut>(tValue)
                : new Optional<TOut>();
        }

        #region IEnumerable<T>
        /// <summary>
        /// Get an <see cref="SCG.IEnumerator{T}"/> that will yield the value if present, or nothing if absent.
        /// </summary>
        /// <returns>A new <see cref="SCG.IEnumerator{T}"/> instance.</returns>
        [Pure]
        public SCG.IEnumerator<T> GetEnumerator()
        {
            var ieValue = (SCG.IEnumerable<T>)FValue ?? new T[0];
            return ieValue.GetEnumerator();
        }
        ...
        {
            return IsPresent && FValue[0].Equals(AValue);
        }
        #endregion
        ...
        /// </summary>
        /// <returns>Either the <see cref="object.ToString()"/> result of the present value, or an "Optional.Absent&lt;<typeparamref name="T"/>&gt;" literal if absent.</returns>
        [Pure]
        ...
        /// <returns>The first <see cref="Optional{T}"/> that is present, or absent if both are absent.</returns>
        [Pure]
        public static Optional<T> operator |(Optional<T> ALeft, Optional<T> ARight) => ALeft.IsPresent ? ALeft : ARight;
        /// <summary>
        ...
        /// <returns>The value of <paramref name="ALeft"/> if present; otherwise <paramref name="ARight"/>.</returns>
        [Pure]
        public static T operator |(Optional<T> ALeft, [NoEnumeration] T ARight) => ALeft.OrDefault(ARight);
    ...
    {
        /// <summary>
        /// Get an <see cref="Optional{T}"/> that is present if the value is not <see langword="null"/>, or absent otherwise.
        /// </summary>
        /// <typeparam name="T">The type of the value.</typeparam>
        /// <param name="AValue">The value.</param>
        /// <returns>An <see cref="Optional{T}"/> wrapping the non-<see langword="null"/> <paramref name="AValue"/>; otherwise absent.</returns>
        [Pure]
        public static Optional<T> IfNotNull<T>([NoEnumeration] this T AValue) =>
            AValue == null ? new Optional<T>() : new Optional<T>(AValue);

        /// <summary>
        /// Get an <see cref="Optional{T}"/> that contains the first value in the <see cref="SCG.IEnumerable{T}"/>, or is absent if none found.
        /// </summary>
        /// <typeparam name="T">The type of the value.</typeparam>
        /// <param name="AEnumerable">The <see cref="SCG.IEnumerable{T}"/>, may not be <see langword="null"/>.</param>
        /// <returns>An <see cref="Optional{T}"/> wrapping the first value in <paramref name="AEnumerable"/>; or is absent if it is empty.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="AEnumerable"/> is <see langword="null"/>.</exception>
        [MustUseReturnValue]
        [ContractAnnotation("AEnumerable: null => halt;")]
        public static Optional<T> IfAny<T>([NotNull] [InstantHandle] this SCG.IEnumerable<T> AEnumerable)
        {
            if (AEnumerable == null) throw new ArgumentNullException(nameof(AEnumerable));

            using (var ieEnumerator = AEnumerable.GetEnumerator())
            {
                if (ieEnumerator.MoveNext())
                    return ieEnumerator.Current;
            }
        ...
        /// <summary>
        /// Get an <see cref="Optional{T}"/> that wraps the result of calling the specified <see cref="Func{TResult}"/>, or is absent if an <see cref="Exception"/> was thrown.
        /// </summary>
        /// <typeparam name="T">The type of the return value.</typeparam>
        /// <param name="AFunc">The <see cref="Func{TResult}"/> to call, may not be <see langword="null"/>.</param>
        /// <returns>An <see cref="Optional{T}"/> wrapping the call result if no exception was thrown; otherwise absent.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="AFunc"/> is <see langword="null"/>.</exception>
        [MustUseReturnValue]
        [ContractAnnotation("AFunc: null => halt;")]
        ...
        /// <summary>
        /// Get an <see cref="Optional{T}"/> that wraps the result of casting to the requested type if possible, or is absent.
        /// </summary>
        /// <typeparam name="T">The type to cast to.</typeparam>
        /// <param name="AObject">The value to cast.</param>
        /// <returns>An <see cref="Optional{T}"/> wrapping the cast result if possible; otherwise absent.</returns>
        [Pure]
        ...
        /// <summary>
        /// Get an <see cref="Optional{T}"/> that wraps the input value if it satisfies a specified <see cref="Predicate{T}"/>.
        /// </summary>
        /// <typeparam name="T">The type of the value.</typeparam>
        /// <param name="AValue">The value.</param>
        /// <param name="APredicate">The <see cref="Predicate{T}"/> to check for, may not be <see langword="null"/>.</param>
        /// <returns>An <see cref="Optional{T}"/> wrapping the value if it matched the predicate; otherwise absent.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="APredicate"/> is <see langword="null"/>.</exception>
        [MustUseReturnValue]
        [ContractAnnotation("APredicate: null => halt")]
        public static Optional<T> If<T>(this T AValue, [NotNull] [InstantHandle] Predicate<T> APredicate)
        {
            if (APredicate == null) throw new ArgumentNullException(nameof(APredicate));
